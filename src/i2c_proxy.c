...
static ssize_t i2c_bus_store(struct device *, struct device_attribute *,
			     const char *, size_t);
static ssize_t i2c_address_show(struct device *, struct device_attribute *,
				char *);
static ssize_t i2c_address_store(struct device *, struct device_attribute *,
				 const char *, size_t);
static ssize_t i2c_data_show(struct device *, struct device_attribute *,
			     char *);
static ssize_t i2c_data_store(struct device *, struct device_attribute *,
			      const char *, size_t);
...
static ssize_t device_fops_write(struct file *, const char __user *, size_t,
				 loff_t *);
...
static ssize_t i2c_bus_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct platform_device *this_platform_device =
		container_of(dev, struct platform_device, dev);
	struct i2c_proxy_device *this_device = container_of(
		this_platform_device, struct i2c_proxy_device, platform_device);
	return sprintf(buf, "%d\n", this_device->bus);
}

static ssize_t i2c_bus_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t i2c_address_store(struct device *dev,
...
static ssize_t i2c_data_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct platform_device *this_platform_device =
		container_of(dev, struct platform_device, dev);
...
static ssize_t i2c_data_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct platform_device *this_platform_device =
		container_of(dev, struct platform_device, dev);
...
static ssize_t device_fops_read(struct file *file, char __user *buf, size_t len,
				loff_t *pos)
{
...
static ssize_t device_fops_write(struct file *file, const char __user *buf,
				 size_t len, loff_t *pos)
{
	int status;
	struct i2c_proxy_device *this_device = container_of(
		file->f_inode->i_cdev, struct i2c_proxy_device, cdev);
...
static int i2c_write_handler(struct device *dev, void *data)
{
	...
	printk(KERN_INFO
	       "i2c_proxy: Found adapter; attempting to send data using i2c bus %d at address 0x%X\n",
...
static int init_device(void)
{
	...
	printk(KERN_INFO
	       "i2c_proxy: Registered chrdev region: major = %d, minor = %d\n",
